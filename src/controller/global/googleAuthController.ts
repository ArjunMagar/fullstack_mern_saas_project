...
class googleAuthController {
  ...
  static callback(req: Request, res: Response) {
    passport.authenticate("google", { session: false }, (err, result) => {
      ...
      //     token,
      //   },
        // Option B: Redirect and pass token to frontend via query
        return res.redirect(`${envConfig.frontendURL}/auth/callback?token=${token}&user=${encodeURIComponent(JSON.stringify(user))}`);
